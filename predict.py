...
def _make_submission(x_raw, y_raw, all_predictions,filename="submission.csv"):
    pd.DataFrame({"product": y_raw, "complaint": x_raw, "predicted": all_predictions.flatten()}).to_csv(
        os.path.join(filename), index=False, float_format="%.5f")
...
def predict_unseen_data():
	...
	if y_test is not None:
		...
		#print("Predicted: {0} Actual: {1}".format(all_predictions, y_test))
		...
		_make_submission(x_raw, y_raw, all_predictions, filename)
